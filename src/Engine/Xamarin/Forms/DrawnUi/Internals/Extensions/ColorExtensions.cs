...
public static partial class ColorExtensions
{
    ...
    {
        var rc = color.R - color.R / 100 * percent;
        if (rc > 1) rc = 1;
        var gc = color.G - color.G / 100 * percent;
        if (gc > 1) gc = 1;
        var bc = color.B - color.B / 100 * percent;
        if (bc > 1) bc = 1;
        var ret = new Color((float)rc, (float)gc, (float)bc, color.A);
        return ret;
    }
    ...
    {
        return color.AddLuminosity((float)(color.Luminosity / 100 * percent));
    ...
    {
        var r = (ushort)(color.R * 255);
        var g = (ushort)(color.G * 255);
        var b = (ushort)(color.B * 255);
        var a = (ushort)(color.A * 255);
