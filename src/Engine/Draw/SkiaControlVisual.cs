...
using DrawnUi.Maui.Drawn.Infrastructure;
using DrawnUi.Maui.Drawn.Infrastructure.Enums;
            ...
            SafePostAction(() =>
            {
                try
                {
                    if (!newvalue)
                    {
                        ...
                        {
                            RenderObject = null; //free gpu memory  
                        }
                    }
                    else
                    {
        ...
        */
        ...
        public virtual SKPoint TranslateInputCoords(SKPoint childOffset, bool accountForCache = true)
        {
            ...
            if (accountForCache && UsingCacheType != SkiaCacheType.None)
            {
                ...
                if (UsingCacheType == SkiaCacheType.ImageDoubleBuffered && OffscreenRenderObject != null)
                {
            ...
            bool useTranlsation = true)
        {
            ...
            if (!ignoreCache && UsingCacheType != SkiaCacheType.None)
            {
                ...
                if (UsingCacheType == SkiaCacheType.ImageDoubleBuffered && OffscreenRenderObject != null)
                {
                    thisOffset.Offset(OffscreenRenderObject.CalculatePositionOffset(LastDrawnAt));
                }
        ...
        /// The existing RenderObject will be copied here to preserve the cache if front is invalidated.
        /// </summary>
        [EditorBrowsable(EditorBrowsableState.Never)]
        ...
        public CachedObject RenderObject
        {
            ...
            set
            {
                ...
                if (_renderObject != value)
                {
                    lock (LockDraw)
                    {
                        if (UsingCacheType == SkiaCacheType.ImageDoubleBuffered && _renderObject != null)
                        {
        ...
        protected virtual bool UseRenderingObject(SkiaDrawingContext context, SKRect recordArea, float scale)
        {
            ...
            //lock (LockDraw)
            {
                ...
                var cacheOffscreen = OffscreenRenderObject;
                ...
                if (cache != null)
                {
                        ...
                        cacheType == SkiaCacheType.GPU && context.Superview?.CanvasView is SkiaViewAccelerated hardware)
                    {
                        ...
                        if ((int)hardware.GRContext.Handle != (int)cache.Surface.Context.Handle)
                        {
                ...
                while (createCacheInBackground != null)
                {
                    try
                    {
                        ...
                        //lock (LockDraw)
                        {
                            ...
                            if (kill != null)
                            {
                                //will crash randomly without this delay
                                //todo investigate alternative synch methods
     ...
     Action<SkiaDrawingContext> action)
        {
                ...
                || useCache == SkiaCacheType.ImageDoubleBuffered)
            {
                ...
                {
                    //hardware context might change if we returned from background..
                    if ((int)hardware.GRContext.Handle != (int)surface.Context.Handle)
                    {
                        needCreateSurface = true;
                    }
                }
